...
router.get("/:id", async (req, res) => {
	let result = await ArtLink.findById(req.params.id);
	// Log results
	console.log("Got the results successfully: " + results);
...
router.post("/", async (req, res) => {
	// Get the document
	let result = await ArtLink.create(req.body);
	...
	res
		.send({ data: result })
		.header("Location", "/art-links/" + result._id)
		.status(201);
});
...
router.delete("/:id", async (req, res) => {
	...
	// Get the collection
	let result = await ArtLink.deleteOne({ _id: new ObjectId(req.params.id)});
	// Log results
...
router.put("/:id", async (req, res) => {
	...
	let result = await ArtLink.findByIdAndUpdate(req.params.id, req.body, { new: true }).exec();
	// Log results
	console.log("Got the results successfully: " + results);
	// Send the result
	res.send({ data: result }).status(200);
